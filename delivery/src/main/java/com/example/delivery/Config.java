...
public class Config {
    ...
    @Bean
    public Queue anonymousInvoicesQueue() {
        return new AnonymousQueue();
    }

    @Bean
    public Binding bindingOrders(@Qualifier("anonymousOrdersQueue") Queue anonymousOrderQueue,
                                 @Qualifier("orders") TopicExchange exchange) {
        return BindingBuilder.bind(anonymousOrderQueue).to(exchange).with("orders.*");
    }

    @Bean
    public Binding bindingInvoices(@Qualifier("anonymousInvoicesQueue") Queue anonymousInvoiceQueue,
                                   @Qualifier("invoices") TopicExchange exchange) {
        return BindingBuilder.bind(anonymousInvoiceQueue).to(exchange).with("invoices.*");
    }
